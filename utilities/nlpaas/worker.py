...
def check_claritynlp_connection():
    ...
    try:
        ...
        logger.info(resp.text.strip('\"'))
        if resp.status_code == 200 and resp.text.strip('\"') == "Welcome to ClarityNLP!":
...
def get_results(job_id: int, name: str = "NLPAAS Job"):
    ...
    url = util.clarity_nlp_api_url + f"job_results/<int:job_id>/<string:job_type>?job_id={job_id}&job_type=phenotype" 
