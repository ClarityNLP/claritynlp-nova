...
# Setup and Installation
    ...
    conda create --name claritynlp python=3.10
...
Change directories to the `native_setup` folder and run:
...
On a Mac with an Intel CPU, start mongo with this command:

    mongod --config /usr/local/etc/mongod.conf

On a Mac with an Apple CPU, start mongo with this command:
...
Can shutdown the mongo server with either:
