...
main_url = read_property("IDENTITY_AND_ACCESS_PROXY_VIRTUAL_HOST", ("main", "url"))
...
solr_report_id_field = read_property("SOLR_REPORT_ID_FIELD", ("solr", "report_id_field"), default="report_id")
solr_source_field = read_property("SOLR_SOURCE_FIELD", ("solr", "source_field"), default="source")
solr_report_date_field = read_property("SOLR_REPORT_DATE_FIELD", ("solr", "date_field"), default="report_date")
solr_subject_field = read_property("SOLR_SUBJECT_FIELD", ("solr", "subject_field"), default="subject")
solr_report_type_field = read_property("SOLR_REPORT_TYPE_FIELD", ("solr", "type_field"), default="report_type")
...
use_redis_caching = read_property("USE_REDIS_CACHING", ("optimizations", "use_redis_caching"), default="false")

use_memory_caching = read_property("USE_MEMORY_CACHING", ("optimizations", "use_memory_cache"), default="true")
use_precomputed_segmentation = read_property("USE_PRECOMPUTED_SEGMENTATION", ("optimizations", "use_precomputed_segmentation"), default="true")
use_reordered_nlpql = read_property("USE_REORDERED_NLPQL", ("optimizations", "use_reordered_nlpql"), default="false")

cql_eval_url = read_property("FHIR_CQL_EVAL_URL", ("local", "cql_eval_url"), key_name="cql_eval_url")
fhir_data_service_uri = read_property("FHIR_DATA_SERVICE_URI", ("local", "fhir_data_service_uri"), key_name="fhir_data_service_uri")
...
fhir_terminology_service_uri = read_property("FHIR_TERMINOLOGY_SERVICE_URI", ("local", "fhir_terminology_service_uri"), key_name="fhir_terminology_service_uri")
fhir_terminology_service_endpoint = read_property("FHIR_TERMINOLOGY_SERVICE_ENDPOINT", ("local", "fhir_terminology_service_endpoint"), key_name="fhir_terminology_service_endpoint")
fhir_terminology_user_name = read_property("FHIR_TERMINOLOGY_USER_NAME", ("local", "fhir_terminology_user"), key_name="fhir_terminology_user_name")
fhir_terminology_user_password = read_property("FHIR_TERMINOLOGY_USER_PASSWORD", ("local", "fhir_terminology_password"), key_name="fhir_terminology_user_password")
...
if redis_host_port and redis_hostname:
    try:
        redis_host_port = int(redis_host_port)
...
def get_cache_compute_count():
    if redis_conn:
        redis_ccc = redis_conn.get("clarity_cache_compute")
        return redis_ccc if isinstance(redis_ccc, int) else 0
    else:
        return 0


def get_cache_query_count() -> int:
    if redis_conn:
        redis_ccq = redis_conn.get("clarity_cache_query")
        return redis_ccq if isinstance(redis_ccq, int) else 0
    else:
        return 0
...
def mongo_client(host: str | None = None,
    ...
    if not port and mongo_port and isinstance(mongo_port, int):
        port = mongo_port

    if not username and mongo_username:
    ...
    if username and len(username) > 0 and password and len(password) > 0:
        ...
        _mongo_client = MongoClient(host=host, port=port, username=username, password=password, socketTimeoutMS=15000, maxPoolSize=500, maxIdleTimeMS=30000)
    else:
