    ...
    if util.use_redis_caching == "true":
        ...
        if not txt:
            ...
            if util.solr_url.startswith("http"):
                ...
                doc = filesystem_data.query_doc_by_id(
                    document_id, solr_url=util.solr_url
                )
...
def document_sections(doc):
    if (
        util.use_precomputed_segmentation == "true"
        and section_names_key in doc
        and len(doc[section_names_key]) > 0
    ):
...
def document_sentences(doc):
    if (
        util.use_precomputed_segmentation == "true"
        and sentences_key in doc
        and len(doc[sentences_key]) > 0
    ):
...
def pipeline_mongo_writer(
    ...
    log(
        "(job={}; pipeline={}) inserted into mongodb {}".format(
            job, pipeline_id, repr(inserted.inserted_id)
        ),
        DEBUG,
    )
...
class BaseCollector(base_model.BaseModel):
    ...
    def run(self, pipeline_id, job, owner, pipeline_type, p_config):
        ...
        try:
            jobs.update_job_status(
                job, util.conn_string, jobs.IN_PROGRESS, "Running Collector"
            )
            self.run_custom_task(
                pipeline_id, job, owner, pipeline_type, p_config, client, db
            )
        except Exception as ex:
            jobs.update_job_status(
                job, util.conn_string, jobs.WARNING, "".join(traceback.format_stack())
            )
            log(ex, ERROR)
        finally:
            client.close()
    ...
    def run_custom_task(
        self, pipeline_id, job, owner, pipeline_type, p_config, client, db
    ):
    ...
    def cleanup(self, pipeline_id, job, owner, pipeline_type, p_config):
        ...
        try:
            jobs.update_job_status(
                job, util.conn_string, jobs.IN_PROGRESS, "Running Collector Cleanup"
            )
            self.custom_cleanup(
                pipeline_id, job, owner, pipeline_type, p_config, client, db
            )
...
class BaseTask(luigi.Task):
        ...
        try:
            ...
            with open(self.output(), "w") as temp_file:
                ...
                if util.solr_url.startswith("http"):
                    ...
                    if (
                        not self.pipeline_config.report_source
                        and len(self.pipeline_config.report_source) == 0
                    ):
    ...
    def output(self):
        output_file = "{0}/pipeline_job{1}_{2}_batch{3}.txt".format(
    ...
    def write_result_data(
        self, temp_file, mongo_client, doc, data: dict, prefix: str = ""
    ):
        inserted = pipeline_mongo_writer(
            mongo_client,
            self.pipeline,
            self.task_name,
            self.job,
    ...
    def write_multiple_result_data(
        self, temp_file, mongo_client, doc, data: list, prefix: str = ""
    ):
        ids = list()
        for d in data:
            inserted = pipeline_mongo_writer(
