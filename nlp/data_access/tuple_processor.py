    ...
    for f_index in range(len(feature_lists)):
        ...
        for patient_id, doc_list in patient_map.items():
            ...
            for q,doc in enumerate(doc_list):
                ...
                #DISPLAY('DOC [{0}]: {1}'.format(q, doc))
                #DISPLAY('FEATURE LIST: {0}'.format(feature_list))
...
def insert_tuple_def_docs(mongo_obj, tuple_def_docs, job_id):
    ...
    DISPLAY('Called insert_tuple_def_docs...')
    DISPLAY('\ttuple_def_docs: {0}'.format(tuple_def_docs))
    DISPLAY('\tjob_id: {0}'.format(job_id))
    
    assert tuple_def_docs is not None
    assert list == type(tuple_def_docs)
...
if __name__ == '__main__':
    ...
    if use_test_db:
        ...
        for statement in TEST_STATEMENTS:
            new_statement, tuple_def_doc = get_tuple_definition(statement)

            print('New statement: "{0}"'.format(new_statement))
            print('tuple_def_doc: {0}'.format(tuple_def_doc))

            if new_statement is None and tuple_def_doc is None:
                # tuple syntax error
                print('Tuple syntax error')
                do_processing = False
            elif tuple_def_doc is None:
                # no tuples found
                print('Found no tuples in the NLPQL')
                do_processing = False
            else:

                assert dict == type(tuple_def_doc)
                print('Tuple definition doc: ')
                for k,v in tuple_def_doc.items():
                    print('\t{0} => {1}'.format(k,v))
                
                # insert tuple definition docs into Mongo
                if not insert_tuple_def_docs(mongo_obj, [tuple_def_doc], job_id):
                    sys.exit(-1)
