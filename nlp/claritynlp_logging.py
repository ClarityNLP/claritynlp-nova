...
class CustomFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        formatter = logging.Formatter(log_fmt, "%Y-%m-%d %H:%M:%S-%Z", style="{")
    ...
    if isinstance(obj, Exception):
        ...
        for t in traceback.format_tb(obj.__traceback__):
            ...
            for line in lines:
                if line.strip() == "":
                    continue
                log("     {}".format(line), level=ERROR)
        return

    repr_obj = repr(obj)
    if "\n" in repr_obj:
        for line in repr_obj.split("\n"):
...
def setup_log(app):
    ...
    if use_gunicorn_logger == "true":
        ...
        the_app.logger.handlers = []
        handler = logging.StreamHandler()
        handler.setFormatter(CustomFormatter())
        the_app.logger.addHandler(handler)
        the_app.logger.setLevel(logging.DEBUG)
